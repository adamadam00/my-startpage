...
function applyTheme(t) {
  ...
  if (t.bgPreset === '30-aurora') {
    ...
    s('--aurora-c1a',      hRgba(ac1, (0.38*ai).toFixed(3)))
    s('--aurora-c2a',      hRgba(ac2, (0.32*ai).toFixed(3)))
    s('--aurora-c3a',      hRgba(ac3, (0.28*ai).toFixed(3)))
    s('--aurora-opacity',  t.bgAuroraOpacity ?? 0.90)
    ...
    s('--aurora-speed-a',  dur(22))
  }

  if (t.bgPreset === '31-deep-ocean') {
    const oi   = (t.bgOceanIntensity ?? 100) / 100
    const dens = t.bgOceanDensity ?? 50
    const tA = Math.round(360 - dens * 2.4) + 'px'
    const tB = Math.round(260 - dens * 1.8) + 'px'
    const tC = Math.round(480 - dens * 3.0) + 'px'
    s('--ocean-bg',       t.bgOceanDeepBg || '#000814')
    s('--ocean-caustic',  hRgba(t.bgOceanCausticC || '#0078c8', (0.22*oi).toFixed(3)))
    s('--ocean-mid',      hRgba(t.bgOceanMidC     || '#003c78', (0.32*oi).toFixed(3)))
    s('--ocean-bio',      hRgba(t.bgOceanBioC     || '#00ffb4', (t.bgOceanBioOpacity ?? 0.08).toFixed(3)))
    s('--ocean-particle', hRgba(t.bgOceanParticleC|| '#64dcff', (t.bgOceanParticleOpacity ?? 0.55).toFixed(3)))
  ...
  if (t.bgPreset === '32-lava-lamp') {
    ...
    s('--lava-c1',    hRgba(t.bgLavaC1 || '#ff4080', (0.65*lo).toFixed(3)))
    s('--lava-c2',    hRgba(t.bgLavaC2 || '#ff8020', (0.60*lo).toFixed(3)))
    s('--lava-c3',    hRgba(t.bgLavaC3 || '#c020ff', (0.58*lo).toFixed(3)))
    s('--lava-c1g',   hRgba(t.bgLavaC1 || '#ff4080', (0.24*lo).toFixed(3)))
    s('--lava-c2g',   hRgba(t.bgLavaC2 || '#ff8020', (0.22*lo).toFixed(3)))
    s('--lava-c3g',   hRgba(t.bgLavaC3 || '#c020ff', (0.20*lo).toFixed(3)))
    s('--lava-opacity', lo)
    s('--lava-speed-a', dur(20))
  ...
  bgEl.textContent = `
    ...
    html.bg-30-aurora::before { animation-duration: ${dur(22)} !important; }
    html.bg-31-deep-ocean::before { animation-duration: ${dur(18)} !important; }
    html.bg-31-deep-ocean::after  { animation-duration: ${dur(12)} !important; }
    html.bg-32-lava-lamp::before { animation-duration: ${dur(20)} !important; }
