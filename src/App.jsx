...
export default function App() {
  ...
  const persistTheme = (t, immediate = false) => {
    ...
    const doSave = async () => {
      ...
      if (!uid) return
      // strip wallpaper (base64 blob) before saving to db
      const { wallpaper, ...themeData } = t
      // add savedAt so remote/local comparison works
  ...
  const toggleAll = () => {
    if (allCollapsed) {
      setTriggerExpand((t) => t + 1); setNotesTrigger(true); setAllCollapsed(false)
    } else {
      setTriggerCollapse((t) => t + 1); setNotesTrigger(false); setAllCollapsed(true)
    }
  }
  ...
  useEffect(() => {
    const onVisible = async () => {
      ...
      try {
        ...
        if (data?.session) { setSession(data.session); await handleRefresh() }
      } catch {}
    }
  ...
  const loadUserSettings = async (force = false) => {
    ...
    try {
      ...
      let localTheme = {}
      try { localTheme = JSON.parse(localStorage.getItem('currenttheme') || '{}') || {} } catch {}
      ...
      if (force) {
        // force = user clicked refresh — prefer remote
        if (hasRemote) {
      ...
      if (!hasLocal && !hasRemote) return // nothing to load, keep DEFAULT_THEME

      if (hasRemote && !hasLocal) {
        const merged = normalizeTheme(remoteTheme, localTheme)
        setThemeState(merged); localStorage.setItem('currenttheme', JSON.stringify(merged))
        return
      }

      if (!hasRemote && hasLocal) {
        // push local up to Supabase
        persistTheme(localTheme, true); return
      }

      // both exist — newest wins
      const localTs = Number(localTheme.savedAt || 0)
      const remoteTs = Number(remoteTheme.savedAt || 0)
      if (remoteTs >= localTs) {
        const merged = normalizeTheme(remoteTheme, localTheme)
        setThemeState(merged); localStorage.setItem('currenttheme', JSON.stringify(merged))
      } else {
        persistTheme(localTheme, true) // push local to remote
      }
  ...
  const handleRefresh = async (workspaceOverride = null) => {
    ...
    try {
      ...
      if (wsErr) { console.error('Workspace refresh error', wsErr.message); return }
      ...
      const [
        ...
        supabase.from('sections').select('*').eq('workspace_id', currentWs)
          .order('col_index', { ascending: true })
          .order('position', { ascending: true }),
        supabase.from('links').select('*').eq('workspace_id', currentWs)
          .order('position', { ascending: true }),
        supabase.from('notes').select('*').eq('workspace_id', currentWs)
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      if (!data?.length) {
        ...
        setWorkspaces([created]); setActiveWs(created.id)
        await handleRefresh(created.id); return
      }
  ...
  useEffect(() => {
    try {
      ...
      if (data.bookmarks?.length) { setBookmarks(data.bookmarks); setBmFolders(data.folders || []) }
    } catch {}
  ...
  const handleSignOut = async () => {
    await supabase.auth.signOut(); setSession(null); setSettingsOpen(false)
  }
  ...
  const exportFullBackup = async () => {
    ...
    for (const ws of wsData || []) {
      const [{ data: secData }, { data: noteData }] = await Promise.all([
        supabase.from('sections').select('*').eq('workspace_id', ws.id)
          .order('col_index', { ascending: true }).order('position', { ascending: true }),
        supabase.from('notes').select('*').eq('workspace_id', ws.id)
          .order('created_at', { ascending: false }),
      ])
      ...
      for (const sec of secData || []) {
        ...
        sectionsWithLinks.push({
          name: sec.name, position: sec.position, col_index: sec.col_index ?? 0,
          collapsed: sec.collapsed,
          links: (secLinks || []).map((l) => ({ title: l.title, url: l.url, position: l.position })),
        })
      }
      backup.workspaces.push({
        name: ws.name, sections: sectionsWithLinks,
  ...
  const exportCSV = async () => {
    const { data: secs } = await supabase.from('sections').select('*').eq('workspace_id', activeWs)
      .order('col_index', { ascending: true }).order('position', { ascending: true })
    const { data: lnks } = await supabase.from('links').select('*').eq('workspace_id', activeWs)
      .order('position', { ascending: true })
      ...
      try {
        ...
        if (ext === 'csv') {
          ...
          for (const line of lines) {
            const [section, title, url] = line.split(',').map((c) => c.trim().replace(/^"/, '').replace(/"$/, ''))
            if (!section || !title || !url) continue
            if (!sectionMap[section]) {
              const { data: sec } = await supabase.from('sections')
                .insert({ user_id: uid, workspace_id: activeWs, name: section, position: pos++, col_index: 0, collapsed: false })
                .select().single()
              sectionMap[section] = { id: sec.id, lpos: 0 }
            }
            await supabase.from('links').insert({
              user_id: uid, workspace_id: activeWs,
              section_id: sectionMap[section].id, title,
              url: url.startsWith('http') ? url : `https://${url}`,
              position: sectionMap[section].lpos++,
            })
          }
          await handleRefresh(activeWs); alert('CSV imported!'); return
        }
        ...
        if (Array.isArray(data)) {
          ...
          for (let i = 0; i < rows.length; i++) {
            const grp = rows[i]
            const { data: sec, error: secErr } = await supabase.from('sections')
              .insert({ user_id: uid, workspace_id: activeWs, name: grp.name ?? grp.title ?? 'Section', position: i, col_index: 0, collapsed: false })
              .select().single()
            if (secErr || !sec) continue
            const lnks = (grp.bookmarks ?? grp.links ?? [])
              .filter((b) => b && (b.url || b.href))
              .map((b, j) => ({ user_id: uid, workspace_id: activeWs, section_id: sec.id, title: b.name ?? b.title ?? 'Link', url: (b.url ?? b.href ?? '').trim(), position: j }))
            if (lnks.length) await supabase.from('links').insert(lnks)
            imported++
          }
          await handleRefresh(activeWs); alert(`Imported ${imported} of ${rows.length} sections.`); return
        }
        ...
        if (data.workspaces && Array.isArray(data.workspaces)) {
          ...
          for (const ws of data.workspaces) {
            const { data: newWs } = await supabase.from('workspaces')
              .insert({ user_id: uid, name: ws.name }).select().single()
            for (let si = 0; si < (ws.sections ?? []).length; si++) {
              const sec = ws.sections[si]
              const { data: newSec } = await supabase.from('sections')
                .insert({ user_id: uid, workspace_id: newWs.id, name: sec.name, position: sec.position ?? si, col_index: sec.col_index ?? sec.colindex ?? 0, collapsed: sec.collapsed ?? false })
                .select().single()
          ...
          await handleRefresh(activeWs); alert('Backup imported!'); return
        }

        if (data.bg || data.text || data.accent) { setTheme({ ...DEFAULT_THEME, ...data }); alert('Theme imported.'); return }
          ...
          <div className="workspace-tabs">
            {workspaces.map((ws) => (
              ...
              >
                {ws.name}
                {workspaces.length > 1 && (
                  <span className="del-ws" onClick={(e) => { e.stopPropagation(); deleteWorkspace(ws.id) }}>×</span>
                )}
              </button>
            ))}
