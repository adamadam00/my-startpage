...
function Row({ label, children, dimLabel = false }) {
  return (
    <div style={{
      ...
      <span style={{ fontSize: '0.82em', color: dimLabel ? 'var(--text-muted)' : 'var(--text-dim)',
      ...
      <div className="settings-panel" data-side={side} style={{ width: 'min(380px, 74vw)' }}>
        ...
        <Group title="Colours" defaultOpen={false} signal={groupSignal}>
          ...
          <Row label="Border opacity">
            <Slider label="Border opacity" val={Math.round((theme.borderOpacity ?? 1) * 100)} min={0} max={100}
              onChange={v => set('borderOpacity', v / 100)} unit="%" />
          </Row>
        ...
        <Group title="Typography" defaultOpen={false} signal={groupSignal}>
          ...
          <Row label="Topbar font size">
            <Slider label="Topbar font size" val={theme.topbarFontSize ?? 12} min={9} max={20} onChange={v => set('topbarFontSize', v)} unit="px" />
          </Row>
          <Row label="Settings font size">
            <Slider label="Settings font size" val={theme.settingsFontSize ?? 13} min={10} max={20} onChange={v => set('settingsFontSize', v)} unit="px" />
          </Row>
        </Group>
        ...
        <Group title="Layout &amp; spacing" defaultOpen={false} signal={groupSignal}>
          ...
          <Row label="Link left padding">
            <Slider label="Link left padding" val={Math.round((theme.linksPaddingH ?? 0.75) * 100)} min={-125} max={200} step={5}
        ...
        <Group title="Cards &amp; borders" defaultOpen={false} signal={groupSignal}>
          <Row label="Card opacity">
            <Slider label="Card opacity" val={Math.round((theme.cardOpacity ?? 1) * 100)} min={0} max={100}
              onChange={v => set('cardOpacity', v / 100)} unit="%" />
          </Row>
          ...
          <div style={{ display: 'flex', gap: '0.25rem', marginBottom: '0.3rem' }}>
            {[{ l: 'Sharp', v: 0 }, { l: 'Normal', v: 10 }, { l: 'Round', v: 20 }].map(p => (
              <button key={p.v} className={`btn-xs${theme.radius === p.v ? ' btn-primary' : ''}`}
                onClick={() => set('radius', p.v)}>{p.l}</button>
            ))}
          </div>
        ...
        {/* ══ VISIBILITY ══ */}
        <Group title="Visibility" defaultOpen={false} signal={groupSignal}>
          <Row label="Show clock">
            <Toggle label="Show clock" checked={!(theme.hideClock ?? false)} onChange={v => set('hideClock', !v)} />
          </Row>
          <Row label="Show weather">
            <Toggle label="Show weather" checked={!(theme.hideWeather ?? false)} onChange={v => set('hideWeather', !v)} />
          </Row>
          <Row label="Show search bar">
            <Toggle label="Show search bar" checked={!(theme.hideSearch ?? false)} onChange={v => set('hideSearch', !v)} />
          </Row>
          <Row label="Show cards">
            <Toggle label="Show cards" checked={!(theme.hideCards ?? false)} onChange={v => set('hideCards', !v)} />
          </Row>
          <Row label="Show notes">
            <Toggle label="Show notes" checked={!(theme.hideNotes ?? false)} onChange={v => set('hideNotes', !v)} />
          </Row>
        </Group>
        ...
        <Group title="Notes" defaultOpen={false} signal={groupSignal}>
          ...
          <Row label="Note card background">
            <ColorPick label="Note card background" value={theme.notesCardBg || '#13131a'} onChange={v => set('notesCardBg', v)} />
          </Row>
          <Row label="Note card bg opacity">
            <Slider label="Note card bg opacity" val={Math.round((theme.notesCardBgOpacity ?? 1) * 100)} min={0} max={100}
              onChange={v => set('notesCardBgOpacity', v / 100)} unit="%" />
          </Row>
          <Row label="Note text colour">
            <ColorPick label="Note text colour" value={theme.notesTextColor || '#e8e8f0'} onChange={v => set('notesTextColor', v)} />
          </Row>
          <Row label="Note text background">
            <ColorPick label="Note text background" value={theme.notesTextBg || '#0c0c0f'} onChange={v => set('notesTextBg', v)} />
          </Row>
        </Group>
        ...
        <Group title="Search" defaultOpen={false} signal={groupSignal}>
          ...
          <Row label="Engine URL">
            <input className="input" style={{ fontSize: '0.78em' }}
              value={theme.searchEngineUrl || 'https://www.google.com.au/search?q='}
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.28rem', padding: '0 0.75rem 0.4rem' }}>
            <button className="btn-xs" onClick={() => set('searchEngineUrl', 'https://www.google.com.au/search?q=')}>Google</button>
          ...
          <Row label="Open search results">
            <select className="input" style={{ fontSize: '0.78em' }}
          ...
          <Row label="Open links in new window">
            <Toggle label="Open links in new window" checked={theme.linksOpenNewWindow ?? true}
              onChange={v => set('linksOpenNewWindow', v)} />
          </Row>
        </Group>
            ...
            {workspaces.map(ws => (
              <div key={ws.id} style={{
                ...
                <span style={{ flex: 1, fontSize: '0.82em', cursor: 'pointer',
                  ...
                  onClick={() => onSetActiveWs(ws.id)}>{ws.name}</span>
        ...
        <Group title="Danger zone" defaultOpen={false} signal={groupSignal}>
          ...
          <div style={{ display: 'flex', gap: '0.4rem', flexWrap: 'wrap' }}>
            <button className="btn btn-danger" style={{ fontSize: '0.78em', padding: '0.28rem 0.7rem' }}
              onClick={onResetWorkspaceLinks}>✕ Clear all sections &amp; links</button>
            <button className="btn btn-danger" style={{ fontSize: '0.78em', padding: '0.28rem 0.7rem' }}
        ...
        <Group title="Bookmarks" defaultOpen={false}>
          ...
          {bookmarkCount > 0 && (
            <Row label="Re-sync">
              <button className="btn btn-primary" style={{ fontSize: '0.8em' }} onClick={forceSync}>↻ Force sync</button>
            </Row>
          )}
