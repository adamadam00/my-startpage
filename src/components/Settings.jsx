...
export default function Settings({ session, userSettings, onSettingsChange, onClose }) {
  ...
  // Apply theme to CSS variables live
  useEffect(() => {
    ...
    r.setProperty('--radius-sm',    Math.max(4, theme.radius - 4) + 'px')
  ...
  const loadSlot = (slot) => {
    if (presets[slot]) {
      setTheme({ ...DEFAULT_THEME, ...presets[slot] })
      setActiveSlot(slot)
    } else {
      setActiveSlot(slot)
    }
  ...
  return (
    <>
      ...
      <div className="settings-panel">
          ...
          <div className="preset-slots">
            {[1,2,3,4].map(slot => (
              <button
                ...
                title={presets[slot] ? `Load: ${slotNames[slot]}` : 'Empty slot — save current theme here'}
              >
                {slotNames[slot]}
              </button>
            ))}
        ...
        {/* ── Card Opacity ── */}
        <div className="settings-section">
          <div className="settings-title">Card Opacity</div>
          <Row label={`${Math.round(theme.cardOpacity * 100)}%`}>
            <input
        ...
        <div className="settings-section">
          ...
          {bgImage && (
            <div style={{ fontSize: '0.75em', color: 'var(--text-muted)' }}>
              ✓ Local image loaded (saved on this device only)
            </div>
          )}
        </div>
        ...
        {/* ── Clock & Search ── */}
        <div className="settings-section">
          <div className="settings-title">Clock & Search</div>
        ...
        <div className="settings-section">
          ...
          <input className="input" placeholder="City name" value={weatherName}
