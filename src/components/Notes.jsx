...
export default function Notes({ notes = [], workspaceId, userId, onRefresh }) {
  ...
  const S = {
    ...
    // ✎ + ✕ buttons — identical fade-in pattern, appear at right edge on row hover
    editBtn: { flexShrink:0, background:'none', border:'none', color:'var(--text-muted)', cursor:'pointer', fontSize:'0.9em', lineHeight:1, padding:'0.15rem 0.15rem', opacity:0, transition:'color 0.15s, opacity 0.15s' },
        ...
        <div style={S.body}>
          ...
          {safeNotes.map((n, i) => {
            ...
            return editing?.id === n.id ? (
              ...
              <div key={n.id} style={S.noteRow(last)}
                onMouseEnter={e => { e.currentTarget.querySelectorAll('[data-btn]').forEach(b => b.style.opacity = '1') }}
                onMouseLeave={e => { e.currentTarget.querySelectorAll('[data-btn]').forEach(b => b.style.opacity = '0') }}>
                <span style={S.noteText} onClick={() => { setEditing(n); setEditText(n.content) }} title="Click to edit">{n.content}</span>
                <button data-btn style={S.editBtn}
                  onClick={() => { setEditing(n); setEditText(n.content) }} title="Edit note"
                  onMouseEnter={e => e.currentTarget.style.color = 'var(--text)'}
                  onMouseLeave={e => e.currentTarget.style.color = 'var(--text-muted)'}>✎</button>
                <button data-btn style={S.delBtn}
                  onClick={() => remove(n.id)} title="Delete note"
